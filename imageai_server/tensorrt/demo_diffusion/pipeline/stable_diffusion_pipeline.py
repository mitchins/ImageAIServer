...
class StableDiffusionPipeline:
    ...
    def calculateMaxDeviceMemory(self):
        ...
        for model_name, engine in self.engine.items():
            # Handle TensorRT-RTX API difference: device_memory_size vs device_memory_size_v2
            if hasattr(engine.engine, 'device_memory_size'):
                max_device_memory = max(max_device_memory, engine.engine.device_memory_size)
            elif hasattr(engine.engine, 'device_memory_size_v2'):
                max_device_memory = max(max_device_memory, engine.engine.device_memory_size_v2)
            else:
                # Fallback for unknown API versions
                max_device_memory = max(max_device_memory, 8 * 1024 * 1024 * 1024)  # 8GB fallback
